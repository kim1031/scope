...
parser = argparse.ArgumentParser(description='Process arguments for running SCOPE-NO')
parser.add_argument('-a', '--application', type=str, 
                    help='application you wish to run')
parser.add_argument('-s', '--starting_config', type=int, 
                    help='starting configurations that SCOPE-NO should run with')
parser.add_argument('-c', '--constraint', type=float, 
                    help='constraint that SCOPE-NO should meet')
parser.add_argument('-t', '--time_interval', type=int, 
                    help='time interval that SCOPE-NO should use')
parser.add_argument('-d', '--save_dir', type=str, 
                    help='directory to save results in')

args = parser.parse_args()

model = SCOPE(X, args.constraint, model_type='gp', gamma=2.25)
